...
func (c *Config) Connect() (*Conn, error) {
	...
	case SecurityTLS:
		conn, err := ldap.DialTLS("tcp", fmt.Sprintf("%s:%d", c.Server, c.Port), &tls.Config{InsecureSkipVerify: c.InsecureSkipVerify, ServerName: c.Server})
		if err != nil {
			return nil, fmt.Errorf("Connection error: %v", err)
		}
		return &Conn{Conn: conn, Config: c}, nil
	case SecurityStartTLS:
		...
		err = conn.StartTLS(&tls.Config{InsecureSkipVerify: c.InsecureSkipVerify, ServerName: c.Server})
		if err != nil {
			return nil, fmt.Errorf("Connection error: %v", err)
		}
		return &Conn{Conn: conn, Config: c}, nil
